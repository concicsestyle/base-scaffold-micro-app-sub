 ...
 * 复制文本到剪贴板
 * @param text 需要复制的文本
 * @returns 返回一个 Promise，成功时返回 true
 */
export const copyText = async (text: string): Promise<boolean | string> => {
...
/**
 * 将JSON字符串转换为大写十六进制编码
 * @param jsonString 要编码的JSON字符串
 * @returns 大写的十六进制编码字符串
 */
export const jsonToHexUpperCase = (jsonString: string): string => {
  if (typeof jsonString !== 'string') {
    throw new Error('输入必须是JSON字符串');
  }

  // 将JSON字符串转换为UTF-8编码的Uint8Array
  const encoder = new TextEncoder();
  const uint8Array = encoder.encode(jsonString);

  // 将每个字节转换为两位十六进制数并转为大写，然后拼接起来
  return Array.from(uint8Array)
    .map(
      (byte) => byte.toString(16).padStart(2, '0').toUpperCase() // 转换为大写
    )
    .join('');
};
