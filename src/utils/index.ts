 ...
 * @param str 待转换的字符串
 * @param upperCaseFirst 是否首字母大写（大驼峰），默认为 false（小驼峰）
 * @param preserveSpaces 是否保留空格不变，默认为 false
 * @param preserveUpperCase 是否保留全大写单词不变，默认为 false
 * @returns 转换后的驼峰格式字符串
 * // 使用示例
 * console.log(toCamelCase('hello_world')); // 输出: helloWorld
 * console.log(toCamelCase('HELLO_WORLD')); // 输出: helloWorld
 * console.log(toCamelCase('HELLO_WORLD', false, false, true)); // 输出: HELLO_WORLD
 * console.log(toCamelCase('hello-world')); // 输出: helloWorld
 * console.log(toCamelCase('hello world')); // 输出: helloWorld
 * console.log(toCamelCase('hello world', false, true)); // 输出: hello world
 * console.log(toCamelCase('Hello World', true)); // 输出: HelloWorld
 * console.log(toCamelCase('123abc')); // 输出: 123abc
 * console.log(toCamelCase('123_abc')); // 输出: 123Abc
 * console.log(toCamelCase('123-abc')); // 输出: 123Abc
 * console.log(toCamelCase('123 ABC')); // 输出: 123Abc
 * console.log(toCamelCase('123 ABC', false, true)); // 输出: 123 ABC
 * console.log(toCamelCase('123 ABC', false, true, true)); // 输出: 123 ABC
 */
export function toCamelCase(str: string, upperCaseFirst = false, preserveSpaces = false, preserveUpperCase = false): string {
  if (!str) return '';

  // 处理特殊情况：全大写单词是否保留不变
  if (preserveUpperCase && /^[A-Z0-9\s]+$/.test(str)) {
    return str;
  }

  // 替换常见的分隔符（除了空格，如果需要保留的话）
  const separatorRegex = preserveSpaces ? /[-_]+(.)?/g : /[-_\s]+(.)?/g;

  // 转换为驼峰格式
  let camelCase = str.replace(separatorRegex, (_, char) => {
