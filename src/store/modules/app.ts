...
export const useAppStore = defineStore('app', () => {
  const sidebarStatus = useStorage('sidebarStatus-' + appWebShortName, '1');
  ...
  const size = useStorage<'large' | 'default' | 'small'>('size-' + appWebShortName, 'small');

  // 语言
  const language = useStorage('language-' + appWebShortName, 'zh_CN');
