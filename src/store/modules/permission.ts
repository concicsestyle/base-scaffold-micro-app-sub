...
import { getRouters } from '@/api/common/admin/menu';
...
export const usePermissionStore = defineStore('permission', () => {
  ...
  const generateRoutes = async (): Promise<RouteRecordRaw[]> => {
    let routerReqData = {};
    if (import.meta.env.VITE_APP_MICRO_APP_FLAG) {
      routerReqData = {
        menuCode: import.meta.env.VITE_APP_MICRO_APP_CODE
      };
    }
    const res = await getRouters(routerReqData);
    const { data } = res;
