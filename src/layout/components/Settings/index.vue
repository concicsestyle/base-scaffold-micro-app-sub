...
const predefineColors = ref(['#4e6bef', '#409eff', '#ff4500', '#ff8c00', '#ffd700', '#90ee90', '#00ced1', '#1e90ff', '#c71585']);
...
const isDark = useDark({
  storageKey: 'useDarkKey-' + appWebShortName,
  valueDark: 'dark-' + appWebShortName,
  valueLight: 'light-' + appWebShortName
});
// 匹配菜单颜色
watch(isDark, () => {
  if (isDark.value) {
...
const saveSetting = () => {
  ...
  const settings = useStorage<LayoutSetting>('layout-setting-' + appWebShortName, defaultSettings);
...
const resetSetting = () => {
  ...
  useStorage<any>('layout-setting-' + appWebShortName, null).value = null;
