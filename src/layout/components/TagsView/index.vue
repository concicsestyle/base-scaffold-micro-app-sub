...
// 微前端应用标题
const microAppSubTitle = import.meta.env.VITE_APP_TITLE;
// 微前端事件操作对象
...
const addTags = () => {
  ...
  if (name) {
    ...
    if (eventCenterWindow) {
      ...
      tagsViewData.data.microAppSubTitle = microAppSubTitle;
      console.log(`【${microAppCode}】微前端子应用向主应用发送tagsView数据`, tagsViewData);
      eventCenterWindow?.forceDispatch(tagsViewData);
    }
  }
};
...
.temp-div::after {
  ...
  right: 0;
  bottom: 0;
  /* 从中间向上下发散的渐变：中间白色透明度高，上下两端透明 */
  background: linear-gradient(to bottom, rgba(255, 255, 255, 0.10) 0%, rgba(255, 255, 255, 0.05) 50%, rgba(255, 255, 255, 0.02) 100%);
  pointer-events: none; /* 不影响原元素交互 */
  z-index: 1;
}
</style>
