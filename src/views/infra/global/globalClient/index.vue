    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button v-hasPermi="['adminGlobal:client:add']" type="primary" plain icon="Plus" @click="handleAdd">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermi="['adminGlobal:client:edit']" type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()">
              修改
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermi="['adminGlobal:client:remove']" type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()">
              删除
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermi="['adminGlobal:client:export']" type="warning" plain icon="Download" @click="handleExport">导出</el-button>
          </el-col>
          <right-toolbar v-model:showSearch="showSearch" @query-table="getList"></right-toolbar>
        </el-row>
      </template>

      <el-table v-loading="loading" :data="clientList" @selection-change="handleSelectionChange" border height="550">
        ...
        <el-table-column label="客户端秘钥" align="center" prop="clientSecret" width="200" />
        <el-table-column label="授权类型" align="center" width="300">
          <template #default="scope">
            <dict-tag :options="client_grant_type" :value="scope.row.grantTypeList" />
          </template>
        </el-table-column>
        <el-table-column label="设备类型" align="center">
          <template #default="scope">
            <dict-tag :options="client_device_type" :value="scope.row.deviceType" />
          </template>
        </el-table-column>
        <el-table-column label="Token活跃超时时间" align="center" prop="activeTimeout" width="200" />
        <el-table-column label="Token固定超时时间" align="center" prop="timeout" width="200" />
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button v-hasPermi="['adminGlobal:client:edit']" link type="primary" icon="Edit" @click="handleUpdate(scope.row)"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button v-hasPermi="['adminGlobal:client:remove']" link type="primary" icon="Delete" @click="handleDelete(scope.row)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-form ref="clientFormRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="授权类型" prop="grantTypeList">
          <el-select v-model="form.grantTypeList" multiple placeholder="请输入授权类型">
            <el-option v-for="dict in client_grant_type" :key="dict.value" :label="dict.label" :value="dict.value"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="设备类型" prop="deviceType">
          <el-select v-model="form.deviceType" placeholder="请输入设备类型">
            <el-option v-for="dict in client_device_type" :key="dict.value" :label="dict.label" :value="dict.value"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="状态">
          <el-radio-group v-model="form.status">
            <el-radio v-for="dict in normal_disable" :key="dict.value" :value="dict.value">
              {{ dict.label }}
            </el-radio>
          </el-radio-group>
        </el-form-item>
...
import { listClient, getClient, delClient, addClient, updateClient, changeStatus } from '@/api/infra/global/globalClient';
import { ClientVO, ClientQuery, ClientForm } from '@/api/infra/global/globalClient/types';
import { CaretRight } from '@element-plus/icons-vue';

const { proxy } = getCurrentInstance() as ComponentInternalInstance;
const { normal_disable } = toRefs<any>(proxy?.useDict('normal_disable'));
const { client_grant_type } = toRefs<any>(proxy?.useDict('client_grant_type'));
const { client_device_type } = toRefs<any>(proxy?.useDict('client_device_type'));
...
const handleExport = () => {
  proxy?.download(
    'infra/admin/global/client/export',
    {
      ...queryParams.value
    },
    `客户端_${new Date().getTime()}.xlsx`
  );
};
