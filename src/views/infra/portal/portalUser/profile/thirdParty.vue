  ...
  <div>
    <el-table :data="auths" style="width: 100%; height: 100%; font-size: 14px">
      ...
      <el-table-column label="头像" width="120" align="center" prop="avatar">
        <template #default="scope">
          <img v-if="scope.row.avatar" :src="scope.row.avatar" style="width: 45px; height: 45px" />
          <img v-else :src="defaultPng" style="width: 45px; height: 45px" />
        </template>
      </el-table-column>
    ...
    <div id="git-user-binding">
      ...
      <div id="authlist" class="user-bind">
        <a class="third-app" href="#" title="使用 统一管理平台 账号授权登录" @click="authUrl('pc_uni')">
          <div class="git-other-login-icon">
            <svg-icon icon-class="xinjia" />
          </div>
          <span class="app-name">统一平台</span>
        </a>
      </div>
    </div>
  </div>
...
import { authUnlock, authBinding } from '@/api/infra/portal/portalSocial/auth';
