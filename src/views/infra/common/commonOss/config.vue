<template>
  <div class="p-2">
    ...
    <el-card shadow="hover">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button v-hasPermi="['adminCommon:ossConfig:add']" type="primary" plain icon="Plus" @click="handleAdd">新增</el-button>
          </el-col>
          <!--          <el-col :span="1.5">-->
          <!--            <el-button v-hasPermi="['adminCommon:ossConfig:edit']" type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()"-->
          <!--              >修改</el-button-->
          <!--            >-->
          <!--          </el-col>-->
          <el-col :span="1.5">
            <el-button v-hasPermi="['adminCommon:ossConfig:remove']" type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()">
              删除
            </el-button>
          </el-col>
      ...
      <el-table v-loading="loading" :data="ossConfigList" @selection-change="handleSelectionChange">
        ...
        <el-table-column v-if="columns[1].visible" label="配置key" align="center" prop="configKey">
          <template #default="scope">
            <el-tag type="primary">{{ scope.row.configKey }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column v-if="columns[2].visible" label="访问站点" align="center" prop="endpoint" />
        <el-table-column v-if="columns[3].visible" label="自定义域名" align="center" prop="domain" />
        <el-table-column v-if="columns[4].visible" label="桶名称" align="center" prop="bucketName">
          <template #default="scope">
            <el-tag type="success">{{ scope.row.bucketName }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" fixed="right" align="center" width="150" class-name="small-padding">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button v-hasPermi="['adminCommon:ossConfig:edit']" link type="primary" icon="Edit" @click="handleUpdate(scope.row)"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button v-hasPermi="['adminCommon:ossConfig:remove']" link type="primary" icon="Delete" @click="handleDelete(scope.row)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 添加或修改文件配置对话框 -->
    <el-dialog v-model="dialog.visible" :title="dialog.title" width="800px" center draggable append-to-body>
      <el-form ref="ossConfigFormRef" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="配置key" prop="configKey">
...
import { listOssConfig, getOssConfig, delOssConfig, addOssConfig, updateOssConfig, changeOssConfigStatus } from '@/api/infra/common/commonOssConfig';
import { OssConfigForm, OssConfigQuery, OssConfigVO } from '@/api/infra/common/commonOssConfig/types';
