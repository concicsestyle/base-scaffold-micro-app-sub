    ...
    <el-card shadow="hover">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button v-hasPermi="['adminCommon:loginInfo:remove']" type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()">
              删除
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermi="['adminCommon:loginInfo:remove']" type="danger" plain icon="Delete" @click="handleClean">清空</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermi="['adminCommon:loginInfo:unlock']" type="primary" plain icon="Unlock" :disabled="single" @click="handleUnlock">
              解锁
            </el-button>
          </el-col>
          <!--          <el-col :span="1.5">-->
          <!--            <el-button v-hasPermi="['adminCommon:loginInfo:export']" type="warning" plain icon="Download" @click="handleExport">导出</el-button>-->
          <!--          </el-col>-->
...
import { list, delLoginInfo, cleanLoginInfo, unlockLoginInfo } from '@/api/infra/common/commonLoginInfo';
import { LoginInfoQuery, LoginInfoVO } from '@/api/infra/common/commonLoginInfo/types';
import { CaretRight } from '@element-plus/icons-vue';

const { proxy } = getCurrentInstance() as ComponentInternalInstance;
const { client_device_type } = toRefs<any>(proxy?.useDict('client_device_type'));
const { success_fail_status } = toRefs<any>(proxy?.useDict('success_fail_status'));
...
const handleExport = () => {
  proxy?.download(
    'infra/admin/common/loginInfo/export',
    {
      ...queryParams.value
    },
