 ...
 * @LastEditors: 王瑶
 * @LastEditTime: 2025-09-01 10:15:10
-->
<template>
    ...
    >
        ...
        >
            ...
            <el-table-column prop="date" label="资质水平" width="160">
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                <!-- 停用的、已经选择了的禁用 -->
                                <el-option
                                    v-for="item of qualifications_level"
                                    :key="item.value"
                                    :value="item.value"
                                    :label="item.label"
                                ></el-option>
                        ...
                        <span v-else>{{
                       row.qualificationsLevel ? qualifications_level.find(x => x.value == row.qualificationsLevel).label : ''
                    }}</span>
                    </el-tooltip>
                </template>
            </el-table-column>
            <el-table-column prop="name" label="有效周期" >
                <template v-slot="{ row }">
                    <div v-if="row.edit" style="display: flex;">
                        ...
                        ></el-input-number>
                        <el-select  style="margin-bottom: 10px;margin-top: 10px;" v-model="row.activeCycleUnit" class="unit-width ml-2">
            ...
            <el-table-column prop="address" label="复检周期" >
                <template v-slot="{ row }">
                    <div v-if="row.edit" style="display: flex;">
                        ...
                        ></el-input-number>
                        <el-select
                            style="margin-bottom: 10px;margin-top: 10px;" v-model="row.recheckCycleUnit" class="unit-width ml-2">
                            <el-option value="0" label="年"></el-option>
                            <el-option value="1" label="月"></el-option>
                            <el-option value="2" label="日"></el-option>
                        </el-select>
            ...
            <el-table-column prop="address" label="使用状态" width="150">
                ...
                <template v-slot="{ row, $index }">
                    ...
                    <span v-else>{{
                        row.useStatus ? use_status.find(x => x.value == row.useStatus).label : ''
                    }}</span>
                </template>
            </el-table-column>
        </el-table>
...
const delteColumn = () => {
    ...
    if (tags) {
        E_Msg.confirm('提示', `第${tags.substring(0, tags.lastIndexOf(','))}行已经被业务使用，不能删除，请确认！`)
        return
    }
...
const startEdit = data => {
    states.tableData = data.qualificationsDetailList.map(x => {
        x.isAdd = 0 // 标识新增还是修改
        return x
    })
    setTableId()
